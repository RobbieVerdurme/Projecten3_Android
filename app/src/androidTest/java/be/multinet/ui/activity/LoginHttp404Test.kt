...
class LoginHttp404Test : KoinTest {
    ...
    @Test
    fun loginWhenHttp404HandlesStatusCode(){
        val username = "username"
        val password = "password"
        NetworkHandler.onNetworkAvailable()
        coEvery { mockUserRepo.login(username,password) } coAnswers { DataOrError(error = DataError.API_NOT_FOUND, data = null)}
        ...
        onView(withText(R.string.userError)).inRoot(withDecorView(not(`is`(activityRule.activity.window.decorView)))).check(matches(isDisplayed()))
    }
}
