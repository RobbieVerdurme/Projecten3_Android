...
class LoginHttp500Test : KoinTest {
    ...
    @Test
    fun loginWhenHttp500HandlesStatusCode(){
        val username = "username"
        val password = "password"
        NetworkHandler.onNetworkAvailable()
        coEvery { mockUserRepo.login(username,password) } coAnswers { DataOrError(error = DataError.API_INTERNAL_SERVER_ERROR, data = null)}
        ...
        onView(withText(R.string.generic_error)).inRoot(withDecorView(not(`is`(activityRule.activity.window.decorView)))).check(matches(isDisplayed()))
    }
