...

{
    ...
    override suspend fun completeChallenge(challenge: Challenge, user: User, rating: Int, feedback: String, token: String): DataOrError<Nothing?> {
        if(NetworkHandler.getNetworkState().value == ConnectionState.CONNECTED){
            val apiResponse: Response<CompleteChallengeResponse>
            try{
                apiResponse = completeChallengeOnServer(challenge.getChallengeId().toInt(),user.getUserId().toInt(),rating,feedback,token)
            }catch (e: IOException){
