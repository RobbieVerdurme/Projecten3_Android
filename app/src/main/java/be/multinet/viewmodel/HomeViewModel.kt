 ...
 */
class HomeViewModel(private val leaderboardRepository: ILeaderboardUserRepoitory, application: Application): AndroidViewModel(application) {
    ...
    private val loadingLeaderboard = MutableLiveData<Boolean>(true)
    private val requestError = MutableLiveData<String>(null)
    private val leaderboard: ArrayList<LeaderboardUser> = ArrayList()
    ...
    private val getLeaderboardErrorMessage: String = application.getString(R.string.leaderboardError)
    ...
    fun getLoadingLeaderboard(): LiveData<Boolean> = loadingLeaderboard
    fun getRequestError(): LiveData<String> = requestError
    fun getLeaderboard(): List<LeaderboardUser> = leaderboard
...
//    }
    ...
    fun loadLeaderboard(user: User){
        this.user = user
        viewModelScope.launch {
            val repositoryResponse = async {
                leaderboardRepository.loadLeaderboard(user!!.getToken(),user.getUserId().toInt())
            }
            val dataOrError = repositoryResponse.await()
            if(dataOrError.hasError()){
                when(dataOrError.error){
                    DataError.API_BAD_REQUEST -> requestError.value = getLeaderboardErrorMessage
                    else -> requestError.value = genericErrorMessage
                }
            }else{
                leaderboard.clear()
                leaderboard.addAll(dataOrError.data)
            }
